...
const VisualizerInner = ({ data }: { data: any }) => {
  ...
  // FIX: Satisfy TS6133 by using an underscore and returning the existing state
  const onEdgesChange: OnEdgesChange = useCallback(
    (_unused) => setEdges((eds) => eds),
    []
  );
  ...
  return (
    <div ref={containerRef} className="h-full w-full bg-[#1a1a1a] relative overflow-hidden">
      ...
      <ReactFlow
        ...
        onEdgesChange={onEdgesChange} // FIX: Assigned here so it's "read"
        minZoom={0.05}
        maxZoom={2}
        onInit={(instance) => {
