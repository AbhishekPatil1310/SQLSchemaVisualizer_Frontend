  ...
  return (
    <section className="rounded-lg border border-slate-300 bg-white p-3 md:p-4 dark:border-sql-700 dark:bg-sql-900/60">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Generated SQL</h3>
        <button
          type="button"
          onClick={handleCopy}
          className="inline-flex items-center gap-1 rounded-md border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-200 dark:border-sql-700 dark:text-slate-300 dark:hover:bg-sql-800"
        >
          <Clipboard size={12} />
          {copied ? 'Copied!' : 'Copy'}
        </button>
      ...
      <pre className="overflow-x-auto rounded-md bg-slate-100 p-3 text-xs text-slate-900 scrollbar-thin dark:bg-sql-950 dark:text-slate-200">
        <code className="font-mono">{query}</code>
      </pre>

      <div className="mt-3 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3">
        <p className="text-xs font-medium text-emerald-700 dark:text-emerald-200">AI Explanation</p>
        <p className="mt-1 text-xs text-emerald-800 dark:text-emerald-100">{explanation}</p>
      </div>
        ...
        <div className="mt-3 space-y-2 text-xs">
          <div
            className={`flex items-center gap-2 rounded-md px-3 py-2 ${
              validation.isValid ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-200' : 'bg-red-500/10 text-red-700 dark:text-red-200'
            }`}
          >
          ...
          {validation.errors.length > 0 ? (
            <div className="rounded-md bg-red-500/10 px-3 py-2 text-red-700 dark:text-red-200">
          ...
          {validation.warnings.length > 0 ? (
            <div className="rounded-md bg-amber-500/10 px-3 py-2 text-amber-700 dark:text-amber-200">
          ...
          {indexes.length > 0 ? (
            <div className="rounded-md border border-violet-500/30 bg-violet-500/10 px-3 py-2 text-violet-700 dark:text-violet-200">
